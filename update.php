...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  $row = Person::selectPersonById('user', $connection->conn, $id);
  ...
  if (empty($emailValue) || empty($fnameValue) || empty($lnameValue)) {
    ...
    $person = new Person($fnameValue, $lnameValue, $emailValue, '');

    //call the static updateClient method and give $client in the parameters
    Person::updatePerson($person, 'user', $connection->conn, $_GET['id']);
